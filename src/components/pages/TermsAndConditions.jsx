...
const TermsAndConditions = () => {
  ...
  // Fetch terms and conditions from API or localStorage
  const fetchTerms = async () => {
    // Check if terms are already saved in localStorage
    const storedTerms = localStorage.getItem("terms");

    if (storedTerms) {
      // If terms are in localStorage, use them
      setTerms(JSON.parse(storedTerms));
    } else {
      // If terms are not in localStorage, fetch them from the API
      try {
        const res = await axios.get(`${Base_Url}/terms`);
        setTerms(res.data);

        // Save the fetched terms in localStorage for future visits
