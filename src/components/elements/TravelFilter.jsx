...
//       try {
//         // Construct query parameters based on formData
//         const params = {
//           airport: formData.departure || "",
...
//           ? `http://localhost:5001/api/deals/?${queryString}`
...
const TravelFilter = () => {
  ...
  //       const response = await axios.get(apiUrl);
  //       setDeals(response.data);
  //       console.log("Fetched Deals:", response.data); // Log after fetching
  //     } catch (err) {
  //       console.error("Error fetching deals:", err);
  //       setError(err.message);
  //     } finally {
  ...
  useEffect(() => {
    fetchDeals();
  }, []); // Empty dependency array to call API only once on mount
  ...
  const handleResetFilters = () => {
    ...
    setDeals([]);
    setError(null);
    navigate("/search", { state: {} }); // Navigate to the search page or reset the state
    setTimeout(function () {
      fetchDeals();
    }, 500);
  };
